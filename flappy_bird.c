...
//     for (int k = 0; k < pipe_size; k += 2)
//     {
//         int red = ((pipes[k + 1] & 0xF8) >> 3) << 11;
//         int green = (((pipes[k] & 0xE0) >> 5)) | ((pipes[k + 1] & 0x7) << 3);
//         int blue = (pipes[k] & 0x1f);

//         short int p = red | ((green << 5) | blue);
//         plot_pixel(cur_pipe.x + (k / 2) % 320, (320 - cur_pipe.low_height) - (k / 2) / 320, p);
//     }
// }
